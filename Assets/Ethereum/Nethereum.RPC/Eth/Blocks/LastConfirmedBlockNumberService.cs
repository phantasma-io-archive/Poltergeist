...
namespace Nethereum.RPC.Eth.Blocks
{
    ...
    public class LastConfirmedBlockNumberService : ILastConfirmedBlockNumberService
    {
        ...
        {
            Phantasma.SDK.Log.Write($"Waiting for current block ({currentBlock}) to be more than {minimumBlockConfirmations} confirmations behind the max block on the chain ({maxBlockOnChain}). Attempt: {attempt}.");
        }
